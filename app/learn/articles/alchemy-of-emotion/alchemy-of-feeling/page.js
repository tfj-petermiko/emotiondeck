...
export default function LearnArticleTableSingle() {
    ...
    async function loadData() {
      try {
        const res = await fetch(
          "/content/learn/articles/alchemy_of_emotion/alchemy_of_feeling/index.json"
        );
  ...
  const handleBackClick = () => {
    setLoadingBack(true);
    setTimeout(() => (window.location.href = "/learn/articles"), 600);
  };
  ...
  // Collect all paragraphs dynamically and keep their proper order
  const paragraphs = [];
  for (let i = 1; i <= 50; i++) {
    const key = `paragraph_${i}`;
    if (article[key]) {
      paragraphs.push(article[key]);
    }
  }
              ...
              <tr className="learn-module-row">
                <td className="learn-module-cell-main">
                  {article.main_paragraph_bold && (
                    <p className="font-semibold mb-2 italic text-gray-300">
                      {article.main_paragraph_bold}
                    </p>
                  )}
                  <br />
                  {paragraphs.map((text, index) => (
                    <p key={index} className="learn-module-text mb-4">
                      {text}
                    </p>
                  ))}
                </td>
